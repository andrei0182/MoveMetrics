...
def test_load_by_sheet_name_uses_gviz_endpoint(mock_get):
    mock_get.return_value = Mock(
        status_code=200, text="Job #,Charged\nA,100", raise_for_status=lambda: None,
    )
    load_google_sheet("abc123", sheet_name="LEADS")
    called_url = mock_get.call_args[0][0]
    assert called_url == (
        "https://docs.google.com/spreadsheets/d/abc123/gviz/tq?tqx=out:csv&sheet=LEADS"
    )
...
def test_strips_column_whitespace(mock_get):
    ...
    df = load_google_sheet("abc123", sheet_name="LEADS")
